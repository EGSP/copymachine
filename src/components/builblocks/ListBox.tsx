...
export function ListBox<T>({
  ...
  const [internalSelectedIndex, setInternalSelectedIndex] = React.useState<number | null>(
    defaultSelectedIndex
  )

  const activeIndex = isControlled ? selectedIndex ?? null : internalSelectedIndex
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "ArrowUp") {
      event.preventDefault()
      const next = current < 0 ? items.length - 1 : clampIndex(current - 1, items.length)
      selectByIndex(next)
      return
    }
  ...
  return (
    <div
      ...
      className={cn("flex min-w-0 flex-col border border-border bg-background", className)}
    >
